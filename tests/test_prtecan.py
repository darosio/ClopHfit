...
class Test_Labelblock:
    ...
    def test_OVER_data(self) -> None:
        ...
        assert (
            str(record[0].message) == "OVER value in A01 well for Label1 of tecanfile: "
        )
    ...
    def test_raise_missing_column(self) -> None:
        ...
        assert str(err.value) == "Cannot build Labelblock: not 96 wells?"
    ...
    def test_raise_missing_row(self) -> None:
        ...
        with pytest.raises(Exception) as err:
            self.lb = prtecan.Labelblock(None, csvl[idxs[0] : len(csvl)])
        assert str(err.value) == "Cannot extract data in Labelblock: not 96 wells?"


class Test_TecanFile:
    ...
    def test_raise_FileNotFoundError(self) -> None:
    ...
    def test_raise_Exception(self) -> None:
        ...
        assert str(err.value) == "No Labelblock found."


class Test_LabelblocksGroup:
...
class Test_Titration:
    ...
    def setup_class(self) -> None:
        ...
        self.tit_cl = prtecan.Titration(ttff('Tecan/list.cl'))
