...
import argparse
import numpy as np
from lmfit import Parameters, Minimizer, minimize, conf_interval, report_fit  # type: ignore
import pandas as pd
...
def main():
    ...
    parser.add_argument("file", help="the file <x y1 y2> without heads")
    parser.add_argument(
        "out_folder", help="The folder to output the .txt and .png files"
    )
    parser.add_argument(
        "-t",
        "--titration-of",
    ...
    parser.add_argument(
        "--boot", dest="nboot", type=int, help="bootstraping using <n> iterations"
    )
    ...
    ci = conf_interval(mini, res, sigmas=[0.674, 0.95])
    print(ci_report(ci))
    # plotting
    ...
    plt.plot(df.x, df.y1, "o", df.x, df.y2, "s", xfit, yfit[0], "-", xfit, yfit[1], "-")
    plt.grid(True)
...
def bootstrap(df, nboot, fit_params, f_out, ttype):
    ...
    dff = pd.DataFrame({"K": kds, "SA1": sa1, "SB1": sb1, "SA2": sa2, "SB2": sb2})
    print(
        "bootstrap: ",
        round(dff.K.quantile(0.025), 3),
        round(dff.K.quantile(0.163), 3),
        round(dff.K.median(), 3),
        round(dff.K.quantile(0.837), 3),
        round(dff.K.quantile(0.975), 3),
    )
