...
def load_csv(fname, dialect=None):
    """Read a csv file.
    ...
    """
    return list(csv.reader(open(fname, encoding="iso-8859-1"), dialect=dialect))


def line_index(lines, search):
    ...
    """
    count = lines.count(search)
    if count <= 1:
        return lines.index(search)  # [].index ValueError if count == 0
    else:
        raise Exception("Many " + str(search) + " lines.")
