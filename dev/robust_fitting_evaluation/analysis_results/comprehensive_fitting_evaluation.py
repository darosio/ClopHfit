...
from src.clophfit.fitting.data_structures import Dataset
...
class FittingMetrics:
    ...
    parameter_errors: dict[str, list[float]] = field(
    ...
    def success_rate(self) -> float:
        ...
        return (
            (self.success_count / self.total_runs * 100) if self.total_runs > 0 else 0.0
        )
...
class ComprehensiveFittingEvaluator:
    ...
    def __init__(self, n_simulations: int = 100, random_seed: int = 42) -> None:
        ...
        self.fitting_functions = {
            ...
            "fit_lm_robust_iterative": lambda ds: fit_lm(
                ds, robust=True, iterative=True
            ),
            ...
            "fit_binding_glob_reweighted": lambda ds: fit_binding_glob_reweighted(
                ds, key="test"
            ),
            "fit_binding_glob_recursive": fit_binding_glob_recursive,
            "fit_binding_glob_recursive_outlier": fit_binding_glob_recursive_outlier,
            "outlier2": lambda ds: outlier2(ds, key="test"),
            # API wrapper functions
            "api_fit_lm": fit_binding_lm,
        ...
        try:
            ...
            if method_name.startswith("api_fit") and method_name.endswith("_bayes"):
                ...
                if initial_result.result and initial_result.result.success:
                    ...
                    msg = "Initial deterministic fit failed for Bayesian method"
                ...
                if method_name in {"outlier2", "api_fit_lm_outlier"}:
                    ...
                    msg = "Skipping method with weighting bug"
                    raise RuntimeError(msg)
            ...
            if (
                fit_result
                and hasattr(fit_result, "result")
                and fit_result.result
                and fit_result.result.success
                and "K" in fit_result.result.params
            ):
    ...
    def run_comprehensive_evaluation(self) -> None:
        ...
        print("🚀 COMPREHENSIVE FITTING EVALUATION")
        print("=" * 60)
        print(
            f"Testing {len(self.fitting_functions)} methods with {self.n_simulations} simulations each"
        )
    ...
    def analyze_results(self) -> pd.DataFrame:
        ...
        print(
            df[["Method", "Success_Rate_%", "Mean_Time_ms", "K_Mean_Error_%"]].head(10)
        )
        print()
        ...
        if len(successful_methods) > 1:
            ...
            if len(k_errors) > 1:
                ...
                try:
                    ...
                    print(
                        f"K parameter accuracy ANOVA: F={f_stat:.3f}, p={p_value:.3f}"
                    )
                        ...
                        for i, method1 in enumerate(method_names[:-1]):
                            for method2 in method_names[i + 1 :]:
                                _t_stat, p_val = stats.ttest_ind(
                                    k_errors[i], k_errors[method_names.index(method2)]
                                )
                                significance = (
                                    "***"
                                    if p_val < 0.001
                                ...
                                print(
                                    f"  {method1} vs {method2}: p={p_val:.3f} {significance}"
                                )
                    else:
    ...
    def create_visualizations(self, df: pd.DataFrame) -> None:
        ...
        ax2 = axes[0, 1]
        param_cols = [
            col for col in df_filtered.columns if col.endswith("_Mean_Error_%")
        ]
        [col.replace("_Mean_Error_%", "") for col in param_cols]
        ...
        for method in df_filtered.head(8)["Method"]:
            if method in self.results:
                ...
                if times:
                    ...
                    time_labels.append(
                        method.replace("fit_", "").replace("binding_", "")[:12]
                    )
        ...
        ax4 = axes[1, 1]
        methods = df_filtered["Method"].apply(
            lambda x: x.replace("fit_", "").replace("binding_", "")[:15]
        )
        ...
        for i, (_bar, rate) in enumerate(zip(bars, success_rates, strict=False)):
            ax4.text(rate + 1, i, f"{rate:.1f}%", va="center", fontsize=8)
    ...
    def generate_recommendations(self, df: pd.DataFrame) -> None:
        ...
        if not reliable_methods.empty:
            fastest_reliable = reliable_methods.loc[
                reliable_methods["Mean_Time_ms"].idxmin()
            ]
        ...
        for i, row1 in df.iterrows():
            for _j, row2 in df.iloc[i + 1 :].iterrows():
                ...
                if success_diff < threshold and k_error_diff < threshold:
                    ...
                    print(
                        f"     Success: {row1['Success_Rate_%']:.1f}% vs {row2['Success_Rate_%']:.1f}%"
                    )
                    print(
                        f"     K Error: {row1['K_Mean_Error_%']:.2f}% vs {row2['K_Mean_Error_%']:.2f}%"
                    )
