...
def generate_diverse_test_cases(n_tests: int) -> list[tuple]:
    ...
    for difficulty, config in difficulty_configs.items():
        for i in range(n_per_difficulty):
            ...
            noise_mult = np.random.uniform(*config["noise_mult_range"])
            pKa = np.random.uniform(*config["pKa_range"])
            ...
            params = RealisticSimulationParameters(
                ...
                outlier_magnitude=np.random.uniform(2.0, 5.0),
            )
...
def display_evaluation_results(results: dict[str, EvaluationResult]) -> None:
    ...
    best_accuracy = min(
        results.items(),
        key=lambda x: x[1].avg_error if x[1].success_rate > 50 else float("inf"),
    )
...
def create_evaluation_plots(results: dict[str, EvaluationResult]) -> None:
